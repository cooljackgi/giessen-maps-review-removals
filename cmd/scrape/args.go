...
func parseArgs(argv []string) (args, error) {
	csvSet := false
	out := args{
		Postcodes:      mapsreview.ActiveCity.DefaultPostcodes,
	...
	for i := 0; i < len(argv); i++ {
		...
		case "--postcodes":
			if value == "" || value == "all" {
				out.Postcodes = mapsreview.ActiveCity.DefaultPostcodes
			} else {
...
Options:
  --postcodes <all|csv>     Default city postcode list. Default: all configured postcodes.
