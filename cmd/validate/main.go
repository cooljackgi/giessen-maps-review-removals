...
func run(args args) error {
	...
	for _, row := range rows {
		...
		if row.Status == "success" {
			...
			if mapsreview.ActiveCity.DistrictsEnabled && mapsreview.DefaultPostcodeSet[mapsreview.StringValue(row.Postcode)] &&
				(row.BezirkID == nil || row.BezirkName == nil) {
				missingDistrict++
			}
		}
		if postcode := mapsreview.StringValue(row.Postcode); postcode != "" && !mapsreview.DefaultPostcodeSet[postcode] {
			outside[postcode]++
		}
	...
	if missingDistrict > 0 {
		warnings = append(warnings, fmt.Sprintf("%d %s success rows are missing %s assignment", missingDistrict, mapsreview.ActiveCity.Name, strings.ToLower(mapsreview.ActiveCity.DistrictLabel)))
	}
	if len(outside) > 0 {
		...
		sort.Strings(parts)
		message := "outside " + mapsreview.ActiveCity.Name + " postcodes: " + strings.Join(parts, ", ")
		if args.StrictCity {
...
Options:
  ...
  --strict-city           Treat outside-city postcodes as errors.`)
			os.Exit(0)
		default:
